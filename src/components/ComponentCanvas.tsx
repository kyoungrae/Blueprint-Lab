...
const ComponentCanvasContent: React.FC = () => {
    ...
    // ── Yjs 프로젝트 입장/퇴장 ───────────────────────────────────────────────
    // 프로젝트 입장 시 Yjs room 연결, 퇴장/언마운트 시 연결 해제
    useEffect(() => {
    ...
    // 위 두 로직은 yjsStore를 통한 CRDT 실시간 동기화로 완전히 대체됩니다.

    const flushAndLeaveProject = useCallback(async () => {
        // ❌ 제거됨: updateProjectData immediate flush (REST PATCH)
        // Yjs가 disconnect 시 서버에 자동으로 최종 상태를 보존합니다.
            ...
            return screens.map((screen) => {
                ...
                const node: RFNode = {
                    ...
                    data: {
                        ...
                        // Yjs CRDT가 모든 변경을 실시간으로 자동 동기화합니다.
                    },
    ...
    }, [deleteScreen, deleteFlow, getNodes, edges]);
                                ...
                                <div className="relative flex w-full h-screen overflow-hidden bg-gray-50">
                                    ...
                                    {blockCollaborationServerDown && (
                                        <div
                                            ...
                                            role="alertdialog"
                                            aria-modal="true"
                                            aria-labelledby="yjs-server-down-title-component"
                                        >
                                            <div className="max-w-md w-full rounded-2xl bg-white shadow-2xl border border-gray-200 p-8 text-center">
                                                <p id="yjs-server-down-title-component" className="text-lg font-black text-gray-900 mb-2">
