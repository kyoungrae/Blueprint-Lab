...
const WbsDevDetailFilterBar: React.FC<WbsDevDetailFilterBarProps> = ({
    ...
    <div className={layout === 'inline' ? 'flex flex-col gap-2' : 'flex flex-col gap-0'}>
        ...
        <div className={`relative ${layout === 'inline' ? '' : 'px-3 pb-2 pt-2 border-b border-gray-100'}`}>
            ...
            <input
                ...
                className={`w-full pl-8 pr-8 py-1.5 text-sm border border-gray-200 rounded-lg bg-white outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20 placeholder:text-gray-400 ${layout === 'inline' ? '' : 'mx-0'}`}
            />
            {menuSearch && (
                <button
                    type="button"
